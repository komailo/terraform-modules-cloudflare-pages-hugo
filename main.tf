...
resource "cloudflare_dns_record" "page_domains" {
  for_each = { for custom_domain in var.custom_domains : custom_domain.name => custom_domain }
  zone_id  = each.value.zone_id
  name     = each.key
  content  = cloudflare_pages_project.cloudflare_pages_hugo_github_project.subdomain
  type     = "CNAME"
...
resource "cloudflare_dns_record" "www_alias" {
  for_each = { for custom_domain in var.custom_domains : custom_domain.name => custom_domain if var.alias_www }
  zone_id  = each.value.zone_id
  name     = "www.${each.key}"
  content  = each.key
